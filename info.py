...
API_ID = int(environ.get('API_ID', '26585721')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '4887f511028d113e5f11d0e6fc583916')  # API Hash from my.telegram.org
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7326542298(8391386178)').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003327242255').split()]  # Channel id for auto indexing (make sure bot is admin)
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003305624387')  # Request channel id (make sure bot is admin)
...
auth_channels     = environ.get("AUTH_CHANNELS", "-1003414909598")# Channels for force sub (make sure bot is admin)
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://trxmethod4_db_user:LKjcXB4atJWl7rVd@cluster0.hqmsci3.mongodb.net/?appName=Cluster0")  # MongoDB URI for the database
